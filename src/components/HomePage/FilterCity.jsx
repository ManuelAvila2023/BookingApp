...
const FilterCity = () => {
  const [cities, getCities] = useFetch(); // Eliminamos loading y error por simplicidad
  const dispatch = useDispatch();

  useEffect(() => {
    const url = "/cities"; // Ruta relativa
    getCities(url);
  }, []);

  const handleCityFilter = (cityId) => {
    const url = `/hotels${cityId ? `?cityId=${cityId}` : ""}`; // Ruta relativa
  ...
  return (
    <article className="filter-country">
      ...
      <ul className="filter-country__list">
        <li className="filter-country__item" onClick={() => handleCityFilter()}>
          All Cities
        </li>
        {cities?.map((city) => (
          <li
