...
const ReservationModal = ({ isOpen, book, onClose }) => {
  ...
  const [, , postApi] = useCrud(); // Solo necesitamos postApi, desestructuramos según necesidad

  const handleSubmit = async (e) => {
    e.preventDefault();
    const comment = e.target.querySelector('[name="comment"]').value;
    const hotelId = book.hotel.id; // Verifica que book.hotel.id exista

    // Validar datos antes de enviar
    if (!hotelId) {
      setMessage("Error: Hotel ID is missing.");
      return;
    }
    if (!comment.trim()) {
    ...
    try {
      ...
      setTimeout(onClose, 2000); // Cierra el modal después de 2 segundos para mostrar el mensaje
    } catch (error) {
      setMessage(
        `Error al enviar la reseña: ${error.response?.data?.message || error.message}. Por favor, verifica el token o permisos.`
      );
  ...
  return (
    <div className="modal">
      <form className="reserve__form" onSubmit={handleSubmit}>
        <div className="reserve__form__x" onClick={onClose}>
          x
        </div>
        ...
        <label className="reserve__form__label reserve__label__rating">
          ...
          <div className="custom-select">
            ...
            >
              <span className="custom-rating-value" data-value={selectedRating.value}>
                {selectedRating.text}
              </span>
            </div>
            <div className="custom-select__options">
              <div
                ...
                data-value="5"
                onClick={() => handleOptionClick(5, "⭐️⭐️⭐️⭐️⭐️")}
              >
                ⭐️⭐️⭐️⭐️⭐️
              </div>
              <div
                className="custom-select__option"
                data-value="4"
                onClick={() => handleOptionClick(4, "⭐️⭐️⭐️⭐️")}
              >
                ⭐️⭐️⭐️⭐️
              </div>
              <div
                className="custom-select__option"
                data-value="3"
                onClick={() => handleOptionClick(3, "⭐️⭐️⭐️")}
              >
                ⭐️⭐️⭐️
              </div>
              <div
                className="custom-select__option"
                data-value="2"
                onClick={() => handleOptionClick(2, "⭐️⭐️")}
              >
                ⭐️⭐️
              </div>
              <div
        ...
        <label className="reserve__form__label">
          ...
          <textarea
            className="reserve__form__value reserve__form__comment__value"
            name="comment"
            required
          ></textarea>
        </label>
        {message && <p className="message">{message}</p>}
        <button className="reserve__form__btn" type="submit">
          Submit
        </button>
      </form>
    </div>
